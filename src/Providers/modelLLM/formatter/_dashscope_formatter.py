...
class _DashScopeFormatterBase(FormatterBase, ABC):
    ...
    input_types: list[str] = Field(
        ...
        description=(
            ...
            "``app/x-thinking`` entries) are used to filter "
            "``DataBlock``\\s; ``app/x-thinking`` enables passing "
    ...
    def supported_input_media_types(self) -> list[str]:
        ...
        ``text/plain`` and ``app/x-thinking``."""
        return [
            t
            for t in self.input_types
            if t not in ("text/plain", "app/x-thinking")
        ]

    @property
    def supports_thinking_input(self) -> bool:
        """Return ``True`` if ``app/x-thinking`` is listed in
        :attr:`input_types`, meaning the model accepts ``reasoning_content``
        in the conversation history."""
        return "app/x-thinking" in self.input_types
