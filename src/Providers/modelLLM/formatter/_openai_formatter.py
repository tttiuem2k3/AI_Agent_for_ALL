# -*- coding: utf-8 -*-
"""The OpenAI formatter for ASOFT."""
import base64
import re
from abc import ABC
...
_OPENAI_INVALID_NAME_CHARS = re.compile(r"[\s<|\\/>]+")


def _sanitize_openai_message_name(name: str) -> str:
    """Return a message name accepted by the OpenAI Chat API."""
    sanitized = _OPENAI_INVALID_NAME_CHARS.sub("_", name).strip("_")
            ...
            for block in msg.get_content_blocks():
                if isinstance(block, TextBlock):
                    ...
                    if content_blocks or tool_calls:
                        msg_openai = {
                            "role": msg.role,
                            "name": _sanitize_openai_message_name(msg.name),
