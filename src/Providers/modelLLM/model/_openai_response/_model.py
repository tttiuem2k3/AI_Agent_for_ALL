...
from Providers.modelLLM.formatter import FormatterBase, OpenAIResponseFormatter
from Runtime.message import Msg, ThinkingBlock, ToolCallBlock, TextBlock
...
class OpenAIResponseModel(ChatModelBase):
    ...
    async def _parse_stream_response(
        ...
        model implementations. Terminal ``response.completed`` and supported
        ``response.incomplete`` events emit an ``is_last=True`` response with
            ...
            if event_type == "response.reasoning_summary_text.delta":
                ...
                if event_type == "response.incomplete":
                    ...
                    if incomplete_reason not in (
                        ...
                        raise RuntimeError(
                            "OpenAI Responses API stream ended incomplete.",
                        )
                ...
                for output_item in getattr(resp, "output", None) or []:
                ...
                # Keep provider error details out of the exception text because
                # upstream sanitization may log or expose the message.
                raise RuntimeError("OpenAI Responses API stream failed.")
