...
:class:`~Runtime.message.DataBlock` (images, video, audio, PDF).
...
class GeminiEmbeddingModel(EmbeddingModelBase[str | DataBlock]):
    ...
    def __init__(
        ...
        Args:
            credential (`CredentialBase`):
                A :class:`~Providers.credential.GeminiCredential`
    ...
    def _data_block_to_part(block: DataBlock) -> Any:
        """Convert a :class:`~Runtime.message.DataBlock` to a Gemini
        ``Part`` object.
        ...
        """
        from google.genai import types
        from Runtime.message import Base64Source, URLSource
