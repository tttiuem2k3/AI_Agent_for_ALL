...
by app-layer services.

:class:`~Service.app.message_bus.MessageBus` itself stays
...
class MessageBusKeys:
    """app-layer key conventions for the message bus."""
    ...
    _PROJECTION_NS = "ASOFT:session:projection:{sid}"
    ...
    _SESSION_EVENTS = "ASOFT:session:events:{sid}"
    ...
    _SESSION_LOCK = "ASOFT:session:lock:{sid}"
    ...
    _WAKEUP_QUEUE = "ASOFT:wakeups"
    _WAKEUP_SIGNAL = "ASOFT:wakeup_signal"
    ...
    _SESSION_CANCEL = "ASOFT:session:cancel"
    _TASK_CANCEL = "ASOFT:task:cancel"
