...
class MessageBus(ABC):  # pylint: disable=too-many-public-methods
    ...
    async def registry_set(
        ...
        *,
        ...
        Args:
            namespace (`str`):
                Registry key (e.g. ``"ASOFT:bg_tasks:sess123"``).
            field (`str`):
    ...
    # (or the standalone functions in Service.app._service) directly.
    #
    ...
    _SESSION_LOCK_KEY = "ASOFT:session:lock:{sid}"
    _SESSION_EVENTS_KEY = "ASOFT:session:events:{sid}"
    _SESSION_CANCEL_KEY = "ASOFT:session:cancel"
    _SESSION_RUN_TTL_SECS = 600
    _SESSION_REPLAY_MAX_LEN = 1000
    _INBOX_KEY = "ASOFT:inbox:{sid}"
    _WAKEUP_QUEUE_KEY = "ASOFT:wakeups"
    _WAKEUP_SIGNAL_KEY = "ASOFT:wakeup_signal"
    _BG_TASKS_KEY = "ASOFT:bg_tasks:{sid}"
    _BG_TASKS_TTL_SECS = 86400
    _TASK_CANCEL_KEY = "ASOFT:task:cancel"
