# -*- coding: utf-8 -*-
"""In-middleware message bus implementation.
...
development, unit tests, and service that want to avoid a Redis
dependency.
...
class InMemoryMessageBus(MessageBus):
    """In-middleware implementation of :class:`MessageBus`.

    Mapping of bus modes to in-middleware structures:
    ...
    def __init__(self) -> None:
        """Initialise empty in-middleware stores."""
    ...
    async def __aenter__(self) -> Self:
        """No-op — nothing to open for in-middleware transport.
    ...
    async def queue_push(
        ...
        *,
        ...
        """Append ``payload`` to the in-middleware drain queue at ``key``.

        ``ttl_secs`` is accepted for API compatibility but ignored — the
        in-middleware implementation does not expire keys.
    ...
    async def log_append(
        ...
        *,
        ...
        Args:
            ...
            ttl_secs (`int | None`, optional):
                Ignored (no-op — in-middleware does not expire keys).
    ...
    async def acquire_lock(
        ...
        *,
        ...
        In-middleware equivalent of a distributed lock.  ``ttl_secs`` is
