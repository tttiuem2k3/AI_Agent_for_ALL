...
from Runtime.middleware import TTSMiddleware
from .._types import (
...
from Runtime.agent import Agent, ModelConfig
...
class ChatService:
    ...
    def __init__(
        ...
        Args:
            ...
            scheduler_manager (`SchedulerManager`):
                app scheduler — passed through to
            ...
            background_task_manager (`BackgroundTaskManager`):
                ...
                :func:`get_toolkit`.
            message_bus (`MessageBus`):
                app-wide message bus. Provides session-level
            ...
            extra_projectors (`list[EventProjector] | None`, optional):
                ...
                :class:`~Service.app._types.EventProjector`.
        """
    ...
    async def run(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "Unable to publish terminal run error for session_id=%s "
                    "publish_error_type=%s",
                    session_id,
                    type(publish_error).__name__,
                )
            # Do not place provider responses, prompts, credentials or raw
            # exception text in logs/events. The error class is sufficient for
            # operational correlation; the public terminal event is sanitized.
            logger.error(
                "ChatService.run failed for user_id=%s session_id=%s "
                "agent_id=%s error_type=%s error_code=%s",
                user_id,
                session_id,
                agent_id,
                type(e).__name__,
    ...
    @staticmethod
    def _sanitize_run_error(error: Exception) -> tuple[str, str, bool]:
        """Return a stable public error without leaking provider payloads."""
        if isinstance(error, HTTPException):
            if error.status_code == 404:
                return (
                    "runtime_configuration_not_found",
                    "AI runtime configuration was not found.",
                    False,
                )
            if 400 <= error.status_code < 500:
                return (
                    "runtime_configuration_invalid",
                    "AI runtime configuration is invalid.",
                    False,
                )
        status_code = getattr(error, "status_code", None)
        if isinstance(status_code, int) and 400 <= status_code < 500:
            parameter = ChatService._safe_provider_parameter(error)
            return (
                "provider_request_invalid",
                (
                    f"The AI provider rejected parameter '{parameter}'."
                    if parameter
                    else "The AI provider rejected the model request."
                ),
                False,
            )
        ...
        return (
            ...
            "The AI model could not complete this run.",
            True,
        )

    @staticmethod
    def _safe_provider_parameter(error: Exception) -> str | None:
        """Extract only a harmless provider parameter name from an SDK error."""
        parameter = getattr(error, "param", None)
        if parameter is None and isinstance(getattr(error, "body", None), dict):
            body = error.body
            nested = body.get("error") if isinstance(body.get("error"), dict) else body
            parameter = nested.get("param")
    ...
    async def _project_event(
        ...
        Each :class:`~Service.app._types.EventProjector` decides
