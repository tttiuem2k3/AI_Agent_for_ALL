...
:meth:`~Service.app.message_bus.MessageBus.session_publish_cancel`
broadcast, picked up locally by
:class:`~Service.app._manager.CancelDispatcher`, which then looks
...
:meth:`~Service.app.message_bus.MessageBus.session_run` (the
...
class ChatRunRegistry:
    ...
    Used by :class:`~Service.app._manager.CancelDispatcher` to find
    and cancel the local task for a given session, and by the lifespan
    to cancel any leftover runs on app shutdown.
    """
    ...
    async def __aexit__(self, *exc: object) -> None:
        """Cancel every still-running task on app shutdown.
