...
class RedisStorage(StorageBase):
    ...
    class KeyConfig(BaseModel):
        ...
        # Record keys
        credential: str = (
            "ASOFT:user:{user_id}:credential:{credential_id}"
        )
        agent: str = "ASOFT:user:{user_id}:agent:{agent_id}"
        session: str = "ASOFT:user:{user_id}:session:{session_id}"

        # Index keys (Redis Sets — store all IDs for a given scope)
        credential_index: str = "ASOFT:user:{user_id}:credentials"
        agent_index: str = "ASOFT:user:{user_id}:agents"
        session_index: str = (
            "ASOFT:user:{user_id}:agent:{agent_id}:sessions"
        )

        # Lookup key: maps (user_id, agent_id) → session_id
        session_lookup: str = (
            "ASOFT:user:{user_id}:agent:{agent_id}:session"
        )

        # Message list key (Redis List — ordered message history per session)
        messages: str = (
            "ASOFT:user:{user_id}:session:{session_id}:messages"
        )
        chat_request: str = (
            "ASOFT:user:{user_id}:session:{session_id}:chat-request:{request_id}"
        )

        schedule: str = "ASOFT:user:{user_id}:schedule:{schedule_id}"
        schedule_index: str = "ASOFT:user:{user_id}:schedules"
        schedule_global_index: str = "ASOFT:schedules"
        schedule_session_index: str = (
            "ASOFT:user:{user_id}:schedule:{schedule_id}:sessions"
        )

        team: str = "ASOFT:user:{user_id}:team:{team_id}"
        team_index: str = "ASOFT:user:{user_id}:teams"
    ...
    async def delete_session(
        ...
        request_pattern = self._key(
            self.key_config.chat_request,
            user_id=user_id,
            session_id=session_id,
            request_id="*",
        )
        request_keys = [key async for key in self._client.scan_iter(request_pattern)]
        if request_keys:
            await self._client.delete(*request_keys)
    ...
    async def reserve_chat_request(
        ...
        """Reserve a request id using Redis SET NX."""
        key = self._key(
            self.key_config.chat_request,
            user_id=user_id,
            session_id=session_id,
        ...
        result = await self._client.set(key, "started", nx=True)
        if result and self.key_ttl is not None:
            await self._client.expire(key, self.key_ttl)
        return bool(result)
    ...
    async def release_chat_request(
        ...
        """Delete a request reservation after a definite scheduling failure."""
        key = self._key(
            self.key_config.chat_request,
            user_id=user_id,
            session_id=session_id,
    ...
    async def set_session_team_id(
        ...
        app normally only mutates via team operations).
