...
class StorageBase(ABC):
    ...
    async def list_all_schedules(self) -> list[ScheduleRecord]:
        ...
        Used on startup to restore the in-middleware scheduler from persisted
    ...
    async def reserve_chat_request(
        ...
        """Atomically reserve a durable chat request id.

        Returns ``True`` only for the first reservation of
        ``(user_id, session_id, request_id)``. The reservation survives
        process restarts and is used by retry/reconciliation callers to
        avoid spawning a second run.
        """
    ...
    async def release_chat_request(
        ...
        """Release a reservation when the run was definitely not scheduled."""
    ...
    async def list_messages(
        ...
        **kwargs: Any,
        ...
        """Return messages for a session with cursor pagination.

        Args:
            user_id (`str`): The owner user id.
            session_id (`str`): The session id.
            offset: Deprecated numeric offset retained for compatibility.
            limit: Maximum number of messages to return.
            before: Message ID cursor for loading older messages.

        Returns:
            Messages in chronological order and whether older messages exist.
        """
