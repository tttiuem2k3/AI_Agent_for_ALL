...
from Providers.credential import CredentialFactory, CredentialBase
...
def create_app(
    ...
    *,
    ...
    title: str = "ASOFT AI Services",
    version: str = __version__,
) -> FastAPI:
    """Create and configure a FastAPI app.

    This is the primary entry point for embedding ASOFT AI Services into an existing
    ...
    Usage — mount onto an existing app::
        ...
        root = FastAPI()
        asoft_app = create_app(
            storage=RedisStorage(),
            message_bus=RedisMessageBus(),
            workspace_manager=LocalWorkspaceManager(),
        )
        root.mount("/ASOFT", asoft_app)
    ...
    Args:
        ...
        extra_agent_middlewares (`AgentMiddlewareFactory | None`, optional):
            ...
            :class:`~Runtime.middleware.MiddlewareBase` instances to
        ...
        extra_agent_tools (`AgentToolFactory | None`, optional):
            ...
            :class:`~Capabilities.tool.ToolBase` instances to register in the
        ...
        custom_subagent_templates (`list[SubAgentTemplate] | None`, optional):
            ...
            :class:`~Service.app._types.SubAgentTemplate` for details.
        custom_agent_cls (`Type[Agent] | None`, optional):
            A custom :class:`~Runtime.agent.Agent` subclass to use
            when assembling agents.  When ``None`` (default), the
            built-in :class:`~Runtime.agent.Agent` is used.
        title (`str`, defaults to ``"ASOFT AI Services"``):
            OpenAPI title shown in the docs UI.
        version (`str`, defaults to the package version):
            API version shown in the docs UI.

    Returns:
        `FastAPI`: A fully configured app ready to serve requests.
    """
    from fastapi import FastAPI
