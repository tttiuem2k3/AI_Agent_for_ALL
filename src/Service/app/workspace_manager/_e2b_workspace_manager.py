...
so callers — notably :class:`Service.app._service.ChatService` —
...
* ``user_id`` / ``agent_id`` are surfaced as extra sandbox metadata
  (``ASOFT.user.id`` / ``ASOFT.agent.id``) so users can
...
from Capabilities.workspace import E2BWorkspace
from Capabilities.workspace._e2b._bootstrap import (
...
class E2BWorkspaceManager(WorkspaceManagerBase):
        ...
        *,
        ...
        Args:
            ...
            sandbox_metadata (`dict[str, str] | None`, optional):
                ...
                ``Capabilities.workspace.id`` / ``ASOFT.user.id`` /
                ``ASOFT.agent.id`` keys. Useful for downstream
    ...
    def _metadata_for(
        ...
        ``E2BWorkspace`` always sets ``Capabilities.workspace.id`` itself;
        we add the user/agent keys here so they show up alongside it
        in the E2B dashboard's metadata filter UI.
        """
        return {
            "ASOFT.user.id": user_id,
            "ASOFT.agent.id": agent_id,
    ...
    async def get_workspace(
        ...
        Args:
            ...
            workspace_id (`str`):
                ...
                ``Capabilities.workspace.id`` metadata.
