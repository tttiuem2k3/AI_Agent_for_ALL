...
from Capabilities.tool import ToolChunk, ToolResponse
...
class ToolOffloadMiddleware(MiddlewareBase):  # pylint: disable=abstract-method
    ...
    Args:
        bg_manager (`BackgroundTaskManager`):
            app-level background task manager. Used to register
            the running asyncio task so :class:`ToolStop` can target it.
        message_bus (`MessageBus`):
            app message bus. The completion callback uses it to
    ...
    async def on_acting(  # type:ignore[override]
        ...
        - A synthetic :class:`~Capabilities.tool.ToolResponse` notifying
