# -*- coding: utf-8 -*-
"""The unified agent class in ASOFT AI Services library."""
import asyncio
import inspect
...
from Common._utils._common import _generate_id, _json_loads_with_repair
...
from Common.exception import AgentOrientedException
...
from Capabilities.workspace import Offloader, WorkspaceBase
...
class Agent:
    ...
    async def _reply_impl(
        ...
        while self.state.cur_iter < self.react_config.max_iters:
            ...
            # ===============================================================
            if action == "reasoning":
                # Compressed the middleware if needed before reasoning
    ...
    async def _reasoning_impl(
        ...
        if completed_response is None:
            raise RuntimeError(
                "Chat model stream ended without a terminal response.",
            )
    ...
    async def _acting(
        ...
        Yields:
            `ToolChunk | ToolResponse`:
                Intermediate :class:`~Capabilities.tool.ToolChunk` objects
                followed by a final :class:`~Capabilities.tool.ToolResponse`.
        """
    ...
    async def _acting_impl(
        ...
        Wraps :meth:`~Capabilities.tool.Toolkit.call_tool` and yields its
    ...
    def _save_to_context(
        ...
        # conversation middleware. Filtering here keeps every downstream walker
