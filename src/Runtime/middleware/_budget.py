# -*- coding: utf-8 -*-
"""Budget control middleware for ASOFT AI Services agents."""
from typing import AsyncGenerator, Callable, TYPE_CHECKING

from Runtime.event import ModelCallEndEvent, ReplyStartEvent, ReplyEndEvent
from Runtime.message import AssistantMsg, HintBlock
...
class ReplyBudgetControlMiddleware(MiddlewareBase):
    ...
    :attr:`~Runtime.agent.AgentState.middle_context`
    ...
    reply ends via a :class:`~ASOFT.event.ReplyEndEvent`.
    ...
    Example::
        ...
        from Runtime.middleware import BudgetControlMiddleware
    ...
    async def on_reply(
        ...
        :class:`~ASOFT.event.ReplyStartEvent`, accumulates cost on each
        :class:`~ASOFT.event.ModelCallEndEvent`, and removes the entry on
        :class:`~ASOFT.event.ReplyEndEvent`.
    ...
    async def on_reasoning(
        ...
        :class:`~Runtime.message.HintBlock` to the last assistant message
        in context (or creates a new
        :class:`~Runtime.message.AssistantMsg`) and overrides
