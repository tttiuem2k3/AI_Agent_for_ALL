    ...
    async def on_reply(
        ...
        with tracer.start_as_current_span(
            ...
            if isinstance(event_arg, ExternalExecutionResultEvent):
                for result in event_arg.execution_results:
                    tool_attrs: dict[str, Any] = {
                        ...
                        SpanAttributes.ASOFT_IS_EXTERNAL_EXECUTION: (
                            True
                        ),
                        **common_attrs,
                    }
            ...
            try:
                ...
                if reply_id:
                    span.set_attribute(
                        SpanAttributes.ASOFT_REPLY_ID,
                        reply_id,
                    )
                if hitl_pending:
                    span.set_attribute(
                        SpanAttributes.ASOFT_HITL_PENDING_TOOLS,
                        json.dumps(hitl_pending, ensure_ascii=False),
                    )
                if external_pending:
                    span.set_attribute(
                        SpanAttributes.ASOFT_EXTERNAL_EXECUTION_PENDING_TOOLS,  # noqa
