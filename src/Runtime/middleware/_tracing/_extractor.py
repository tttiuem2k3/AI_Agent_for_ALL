# -*- coding: utf-8 -*-
"""Extract attributes from ASOFT AI Services components for OpenTelemetry tracing."""
import inspect
from typing import Any, Dict, TYPE_CHECKING

from Runtime.message import Msg, ToolCallBlock
...
from Providers.modelLLM.model import ChatResponse, ChatModelBase
...
def _get_provider_name(instance: "ChatModelBase") -> str:
    ...
    This follows the implementation pattern from ASOFT-java PR #73.
...
def _get_tool_definitions(
    ...
    Converts ASOFT AI Services/OpenAI nested tool format to OpenTelemetry GenAI
...
def _get_llm_response_attributes(
    ...
    if hasattr(chat_response, "usage") and chat_response.usage:
        ...
        if cache_input:
            attributes[
                SpanAttributes.ASOFT_CACHE_INPUT_TOKENS
            ] = cache_input

        cache_creation = usage.cache_creation_input_tokens
        if cache_creation:
            attributes[
                SpanAttributes.ASOFT_CACHE_CREATION_INPUT_TOKENS
            ] = cache_creation
...
def _get_agent_messages(
    ...
    """Convert ASOFT AI Services message(s) to standardized parts format.

    Transforms Msg objects into OpenTelemetry GenAI format.

    Args:
        msg (`Msg | list[Msg]`):
            ASOFT AI Services message object or list of message objects with
            content blocks.
...
def _get_agent_request_attributes(
    ...
    if inputs is not None:
        if isinstance(inputs, (Msg, list)):
            ...
            attributes[
                SpanAttributes.ASOFT_INCOMING_EVENT_TYPE
            ] = "user_confirm_result"
