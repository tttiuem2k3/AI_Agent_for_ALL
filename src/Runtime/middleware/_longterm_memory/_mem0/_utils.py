...
These are stateless adapters that translate between ASOFT AI Services and
...
from Runtime.event import ExternalExecutionResultEvent, UserConfirmResultEvent
...
def _extract_memory_texts(raw: Any) -> list[str]:
    """Flatten a mem0 search response into a list of middleware strings.

    Tolerates the common shapes:
      - ``{"results": [{"middleware": str, ...}, ...]}`` (current OSS / Platform)
      - ``[{"middleware": str, ...}, ...]`` (legacy / variations)
    ...
    for item in results:
        if isinstance(item, str):
            ...
            text = item.get("middleware") or item.get("text")
            if text:
                out.append(str(text))
    return out
