# -*- coding: utf-8 -*-
"""mem0-backed long-term middleware middleware for ASOFT AI Services agents.
...
from Runtime.event import ReplyStartEvent
...
class Mem0Middleware(MiddlewareBase):
    """ASOFT AI Services middleware that adds long-term middleware backed by
    `mem0 <https://github.com/mem0ai/mem0>`_.

    Two construction paths:

    1. **Models** — pass an ASOFT AI Services ``chat_model`` and
       ...
       middleware extraction and embedding both go through your ASOFT AI Services
       models.
    ...
    def __init__(
        ...
        *,
        ...
        - **Models + ``mem0_config``** — same but starts from your
          ...
          slots are overridden with the ASOFT AI Services adapters, every
        ...
        Args:
            user_id:
                The mem0 ``user_id`` for middleware namespacing. Required.
            client:
            ...
            chat_model:
                The ASOFT AI Services chat model mem0 should use for middleware
                extraction. Required if ``client`` is not given and
                ``mem0_config`` does not already supply an LLM.
            embedding_model:
                The ASOFT AI Services embedding model mem0 should use to
            ...
            mem0_config:
                ...
                embedding through ASOFT AI Services. Mutually exclusive with
                ``client``.
            mode:
                ...
                - ``"static_control"``: middleware searches mem0
                  ...
                  ``AssistantMsg(name="middleware")``, and writes the new
                ...
                Defaults to ``"both"``, enabling automatic context injection
                and explicit memory tools.
            agent_id:
    ...
    async def on_reply(
        ...
        # reply path — the agent decides when to invoke the middleware
        ...
        # static_control / both follow the ASOFT AI Services memory contract:
    ...
    # ------------------------------------------------------------------
    # Hook: on_system_prompt (advertise middleware tools to the LLM)
    # ------------------------------------------------------------------
    async def on_system_prompt(
        ...
        """Append middleware-tool instructions to the system prompt.
        ...
        Returns:
            `str`:
                ...
                prompt with middleware-tool instructions appended.
        """
    ...
    async def list_tools(self) -> list["ToolBase"]:
        """List middleware tools provided by this middleware.
    ...
    def _resolve_client(
        *,
        ...
        if client is None:
            ...
            from ._asoft_adapter import build_mem0_config
    ...
    async def _async_search(
        ...
        *,
        ...
        """Search mem0 and normalize the result into middleware strings.
        ...
        Returns:
            `list[str]`:
                Retrieved middleware texts extracted from mem0's response.
        """
    ...
    async def _async_add(
        ...
        *,
        ...
        Args:
            ...
            infer (`bool`, optional):
                Whether mem0 should run middleware extraction. ``False`` asks
    ...
    async def _async_add_with_fallback(
        ...
        *,
        ...
        persists *something* — matching the ASOFT AI Services
        ``record_to_memory`` "always save" contract.

        Historical note — why this is 2 tiers, not 3
        ---------------------------------------------
        ASOFT AI Services uses a 3-tier fallback:
        ...
        middleware-worthy but the caller wants the bytes persisted anyway.
        """
    ...
    def _build_memory_message(self, memories: list[str]) -> Msg:
        ...
        Args:
            memories (`list[str]`):
                Retrieved middleware texts to expose to the model.
