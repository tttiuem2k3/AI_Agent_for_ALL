...
Shape and behavior follow the ASOFT AI Services
``retrieve_from_memory`` / ``record_to_memory`` contract
(multi-keyword parallel search, fallback write, verbose result
text) — adapted to ASOFT AI Services 2.x's tool conventions
...
class _Mem0MemoryToolBase(ToolBase):
    ...
    Middleware-provided middleware tools are part of the agent's standard
    ...
    async def check_permissions(
        ...
        **_kwargs: Any,
        ...
        return PermissionDecision(
            ...
            message="auto-allowed: mem0 long-term middleware tool",
        )
...
class _SearchMemoryTool(_Mem0MemoryToolBase):
    ...
    async def __call__(
        ...
        """Retrieve the middleware based on the given keywords.
        ...
        Args:
            keywords (list[str]):
                ...
                middleware). Each keyword is issued as an independent query
                against the middleware store; results are merged and
                deduplicated.
            limit (int):
        ...
        try:
            ...
            return _error_chunk(f"Error retrieving middleware: {e}")
    ...
    async def __call__(
        ...
        Args:
            thinking (str):
                ...
                the middleware store — only ``content`` is. Use it to
        ...
        try:
            ...
            return _error_chunk(f"Error recording middleware: {e}")

        rationale = f" (rationale: {thinking})" if thinking else ""
        return _text_chunk(
            f"Successfully recorded to middleware{rationale} → {result}",
        )
