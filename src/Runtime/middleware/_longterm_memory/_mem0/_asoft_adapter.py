# -*- coding: utf-8 -*-
"""Adapters that let mem0 drive its middleware extraction with the user's
existing ASOFT AI Services chat / embedding model — instead of building yet
another OpenAI / Anthropic / Ollama client just for mem0.

Two pieces:

- :class:`ASOFTLLM` — implements ``mem0.llms.base.LLMBase`` by
  delegating to an ``ASOFT.model.ChatModelBase`` instance.
- :class:`ASOFTEmbedding` — implements
  ``mem0.embeddings.base.EmbeddingBase`` by delegating to an
  ``ASOFT.embedding.EmbeddingModelBase`` instance.

mem0 calls these synchronously; ASOFT AI Services models are async. We bridge
...
ASOFT AI Services models:

    from mem0 import AsyncMemory
    from Runtime.middleware._longterm_memory._mem0._asoft_adapter \\
...
class _AsyncBridge:
    ...
    sync, but ASOFT AI Services models are async. The bridge owns one event
...
class ASOFTLLM(LLMBase):
    """mem0 ``LLMBase`` backed by an ASOFT AI Services ``ChatModelBase``.

    Pass your ASOFT AI Services model into ``config["model"]``; mem0's middleware
    extraction calls then route through it. Both streaming and
    non-streaming ASOFT AI Services models are accepted (streaming responses are
    ...
    def __init__(
        ...
        """Initialize the ASOFT AI Services LLM for mem0."""
        super().__init__(config)
        if self.config.model is None:
            raise ValueError(
                "ASOFTLLM requires `model` in the config to be an "
                "ASOFT AI Services ChatModelBase instance.",
            )
        if not isinstance(self.config.model, ChatModelBase):
            raise TypeError(
                f"ASOFTLLM `model` must be a ChatModelBase, got "
                f"{type(self.config.model).__name__}.",
            )
        self._asoft_model: ChatModelBase = self.config.model
    ...
    def generate_response(
        ...
        """mem0 ``LLMBase`` entry — runs the ASOFT AI Services chat model
        synchronously and returns str (or dict with tool_calls when
        ``tools`` is given)."""
        as_messages = _convert_messages_to_asoft(messages)
        if not as_messages:
            raise ValueError(
                "ASOFTLLM received no usable messages "
                "(empty list or all roles unrecognized).",
            )

        response = self._bridge.run(
            _await_chat(self._asoft_model, as_messages, tools),
        )
...
async def _await_chat(
    ...
    """Call the ASOFT AI Services chat model, handling both streaming and
    non-streaming returns."""
    result = await model(messages, tools=tools)
    # Streaming model — drain the generator and keep the final chunk,
    # which carries the complete content per ASOFT AI Services's streaming
    ...
    if isinstance(result, AsyncGenerator):
        ...
        if last is None:
            raise RuntimeError(
                "ASOFT AI Services streaming model yielded no chunks.",
            )
        return last
    return result


def _convert_messages_to_asoft(
    messages: list[dict[str, str]],
) -> list[Msg]:
    """mem0 hands us OpenAI-style ``[{"role", "content"}, ...]`` dicts;
    ASOFT AI Services wants ``Msg`` objects."""
    out: list[Msg] = []
    for m in messages:
        role = m.get("role")
...
def _parse_chat_response(
    ...
    """Flatten an ASOFT AI Services ``ChatResponse`` into the str/dict shape
    ...
    for block in response.content or []:
        ...
        if block_type == "text":
            ...
            # ASOFT AI Services 2.0 stores tool args as a JSON string; mem0
...
class ASOFTEmbedding(EmbeddingBase):
    """mem0 ``EmbeddingBase`` backed by an ASOFT AI Services
    ``EmbeddingModelBase``."""
    ...
    def __init__(
        ...
        if self.config.model is None:
            raise ValueError(
                "ASOFTEmbedding requires `model` in the config "
                "to be an ASOFT AI Services EmbeddingModelBase instance.",
            )
        if not isinstance(self.config.model, EmbeddingModelBase):
            raise TypeError(
                f"ASOFTEmbedding `model` must be an "
                f"EmbeddingModelBase, got "
                f"{type(self.config.model).__name__}.",
            )
        self._asoft_model: EmbeddingModelBase = self.config.model
        self._bridge = _AsyncBridge()
    ...
    def embed(
        ...
        """mem0 ``EmbeddingBase`` entry — runs the ASOFT AI Services embedding
        model synchronously and returns the first vector."""
        text_list = [text] if isinstance(text, str) else list(text)
        response = self._bridge.run(self._asoft_model(text_list))
        if not response.embeddings:
            raise RuntimeError(
                "ASOFT AI Services embedding model returned no embeddings.",
            )
        # ASOFT AI Services EmbeddingResponse.embeddings is List[List[float]];
        # mem0 expects a single vector for a single-text call.
        return response.embeddings[0]


# ----------------------------------------------------------------------
# Build a mem0 MemoryConfig wired to ASOFT AI Services models
# ----------------------------------------------------------------------

# The provider name we register under in mem0's factory + config layer.
_ASOFT_PROVIDER = "ASOFT"
...
def build_mem0_config(
    *,
    ...
    """Build a ``mem0.configs.base.MemoryConfig`` wired to ASOFT AI Services
    ...
    2. ``build_mem0_config(mem0_config=cfg, chat_model=...,
       ...
       the ASOFT AI Services adapters. Use this when you want a non-default
       vector store / history DB / reranker but still want ASOFT AI Services
       to drive middleware extraction. Either or both of ``chat_model``
    ...
    3. ``build_mem0_config(mem0_config=cfg)`` — pass-through; the
       ASOFT AI Services adapters are registered (cheap) but no fields are
       overridden.

    Why a helper? Two private layers inside mem0 reject anything
    outside its built-in provider list, so plugging in ASOFT AI Services
    requires both:

    1. Adding ``"ASOFT"`` to ``LlmFactory.provider_to_class`` /
    ...
    2. Bypassing the hardcoded provider whitelist in
       ...
       ``"ASOFT"``. Other provider names are rejected by these
       subclasses, matching the fact that this helper is only for wiring
       the ASOFT AI Services adapters.

    Args:
        chat_model:
            The ASOFT AI Services ``ChatModelBase`` mem0 should use for middleware
            extraction. Required when ``mem0_config`` is not given.
        embedding_model:
            The ASOFT AI Services ``EmbeddingModelBase`` mem0 should use to
    ...
    _register_asoft_provider()
    llm_cfg_cls, emb_cfg_cls = _asoft_config_classes()
...
def _register_asoft_provider() -> None:
    """Plug the ASOFT AI Services adapter classes into mem0's factory dicts
    under provider name ``"ASOFT"``. Idempotent."""
    from mem0.utils.factory import EmbedderFactory, LlmFactory

    LlmFactory.provider_to_class[_ASOFT_PROVIDER] = (
        f"{__name__}.ASOFTLLM",
        BaseLlmConfig,
    )
    EmbedderFactory.provider_to_class[
        _ASOFT_PROVIDER
    ] = f"{__name__}.ASOFTEmbedding"


def _asoft_config_classes() -> tuple[type, type]:
    """Return ``LlmConfig`` / ``EmbedderConfig`` subclasses whose
    validator allows ``provider="ASOFT"`` (and only that — every
    ...
    class _ASOFTLlmConfig(LlmConfig):
        """``LlmConfig`` subclass that accepts the ASOFT AI Services provider."""

        @field_validator("config")
        @classmethod
        def validate_config(cls, v: Any, values: Any) -> Any:
            """Allow ``provider == "ASOFT"``; reject everything
            else with mem0's original error."""
            provider = values.data.get("provider")
            if provider == _ASOFT_PROVIDER:
                return v
            raise ValueError(f"Unsupported LLM provider: {provider}")

    class _ASOFTEmbedderConfig(EmbedderConfig):
        """``EmbedderConfig`` subclass that accepts the ASOFT AI Services
        provider."""
    ...
    return _ASOFTLlmConfig, _ASOFTEmbedderConfig
