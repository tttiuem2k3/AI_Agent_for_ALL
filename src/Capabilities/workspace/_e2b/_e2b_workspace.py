...
Mirrors :class:`Capabilities.workspace.DockerWorkspace` but swaps the
...
* **Bootstrap.** First-time provisioning installs uv + a gateway venv
  + ASOFT (``--no-deps``) and uploads the gateway script. We
...
from ._bootstrap import (
    ...
    render_install_asoft_cmd_dev,
    render_install_asoft_cmd_released,
)
...
class E2BWorkspace(WorkspaceBase):
        ...
        *,
        ...
        Args:
            workspace_id (`str | None`, optional):
                ...
                sandbox's ``Capabilities.workspace.id`` metadata for
    ...
    async def initialize(self) -> None:
        ...
        2. If not found, ``AsyncSandbox.create(...)`` provisions a
           ...
           (uv → gateway venv → ASOFT ``--no-deps`` → upload
           gateway script).
    ...
    async def _run_bootstrap(self) -> None:
        """Provision a fresh sandbox: uv → venv → ASOFT → script.
        ...
        if _is_released_install():
            ...
            install_cmd = render_install_asoft_cmd_released(
                _asoft_version(),
            )
        else:
            ...
            install_cmd = render_install_asoft_cmd_dev()

        commands = bootstrap_commands(
            extra_pip=self.extra_pip,
            install_asoft_cmd=install_cmd,
        )
        for cmd in commands:
