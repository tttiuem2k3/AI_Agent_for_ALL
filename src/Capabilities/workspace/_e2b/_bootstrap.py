...
gateway base requirements, and the ``ASOFT`` package. After this
...
* **released** — ``ASOFT`` is in site-packages on the host;
...
``Capabilities.mcp.MCPClient`` whose transitive needs are
``mcp / pydantic / httpx`` (already installed via the gateway base
requirements). Pulling ASOFT's full dep tree drags in heavy /
...
GATEWAY_HOME = f"{SANDBOX_USER_HOME}/.ASOFT"
...
METADATA_WORKSPACE_ID_KEY = "Capabilities.workspace.id"

#: Tarball drop point for dev-mode ``ASOFT`` source uploads.
...
DEV_SRC_TAR = f"{GATEWAY_HOME}/asoft_ai_services_src.tar"
DEV_SRC_DIR = f"{GATEWAY_HOME}/asoft_ai_services_src"
...
def build_source_tarball() -> bytes:
    """Tar up the ASOFT source tree for dev-mode upload.

    Returns the tar bytes (uncompressed); the sandbox will untar with
    ``tar -xf`` (no ``-z``).
    """
    root = _asoft_source_root()
    buf = io.BytesIO()
...
def bootstrap_commands(
    *,
    ...
    Args:
        ...
        install_asoft_cmd: Shell command that installs
            ``ASOFT`` into the gateway venv. Built per install
            mode by :func:`render_install_asoft_cmd` (released)
            or :func:`render_install_asoft_cmd_dev` (dev).
    ...
    return [
        ...
        # 5. ASOFT itself (mode-dependent).
        install_asoft_cmd,
    ]


def render_install_asoft_cmd_released(version: str) -> str:
    """Released-mode install: pin the same version as the host."""
    return (
        f"{UV_BIN} pip install --python {GATEWAY_VENV_PY} "
        f"--no-deps 'asoft-ai-services=={version}'"
    )


def render_install_asoft_cmd_dev() -> str:
