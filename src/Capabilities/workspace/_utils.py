...
Pure functions for detecting the local ``ASOFT`` install (released
...
#: same tuple into the gateway venv before adding ``ASOFT`` itself.
...
#: Basename set excluded when packaging the ASOFT source tree
...
# ── ASOFT install detection ───────────────────────────────────


def _asoft_module_path() -> Path:
    """Return the source/package root of the installed distribution.

    Returns:
        `Path`:
            ``src`` in development or ``site-packages`` when installed.
    """
    import Capabilities  # local import — keeps module import cheap

    file = getattr(Capabilities, "__file__", None)
    if not file:
        raise RuntimeError(
            "Capabilities has no __file__; cannot locate package root",
        )
    return Path(file).resolve().parent.parent


def _is_released_install() -> bool:
    """Return ``True`` if the distribution lives in site-packages.
    ...
    """
    parts = _asoft_module_path().parts
    return "site-packages" in parts or "dist-packages" in parts


def _asoft_version() -> str:
    """Return the installed ASOFT AI Services version string.
    ...
    if not version:
        try:
            ...
            version = _v("asoft-ai-services")
        except Exception as e:  # noqa: BLE001
            raise RuntimeError(
                "cannot determine ASOFT version",
            ) from e
    return version


def _asoft_source_root() -> Path:
    """Locate the project root containing ``pyproject.toml`` + ``src/``.

    Only valid in dev mode. Walks up from the package directory until
    a ``pyproject.toml`` is found alongside a ``src/`` directory.

    Returns:
        `Path`:
            The project root path.
    """
    pkg = _asoft_module_path()
    for parent in [pkg, *pkg.parents]:
        if (parent / "pyproject.toml").is_file() and (
            ...
            return parent
    raise RuntimeError(
        f"cannot locate ASOFT project root from {pkg}",
    )
...
def _read_gateway_script_bytes() -> bytes:
    ...
    The script ships at
    ``Capabilities/workspace/_mcp_gateway/_mcp_gateway_app.py``. Both
    ...
    the heavy ``Capabilities.workspace.__init__`` import graph.
    """
    return (
        _res.files("Capabilities.workspace._mcp_gateway")
...
def _read_glob_helper_bytes() -> bytes:
    ...
    The script ships at
    ``Capabilities/tool/_builtin/_scripts/_glob_helper.py``. Both Docker
    ...
    """
    return (
        _res.files("Capabilities.tool._builtin._scripts")
