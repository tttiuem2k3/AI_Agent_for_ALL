...
* MCP servers run *inside* the container behind a FastAPI gateway
  (see :mod:`Capabilities.workspace._mcp_gateway`); the host talks to it
...
Persistence model mirrors :class:`Capabilities.workspace.LocalWorkspace`:
...
class DockerWorkspace(WorkspaceBase):
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            base_image (`str`, defaults to `DEFAULT_BASE_IMAGE`):
                ...
                Dockerfile (uv venv + ASOFT install + optional
                node + ``extra_pip``).
    ...
    async def initialize(self) -> None:
        ...
        5. Drop ``gateway.config.json`` into the container, launch the
           gateway via ``python -m Capabilities.workspace._mcp_gateway``,
    ...
    async def _create_and_start_container(self) -> None:
        ...
        * ``Capabilities.workspace.id`` label for later discovery.
        ...
        config: dict[str, Any] = {
            ...
            "Labels": {
                ...
                "Capabilities.workspace.id": self.workspace_id,
            },
        ...
        # newly-bind-mounted host workdir ASOFT-shaped on first use).
        await self._backend.exec_shell(
            [
                "mkdir",
                "-p",
    ...
    async def _start_gateway_process(self) -> None:
        ...
        auto-import ``Capabilities.workspace.__init__`` and the heavy
