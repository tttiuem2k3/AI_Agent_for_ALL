...
* **released** — when ``ASOFT`` is in site-packages, the container
...
* **dev** — when running from a source checkout, the project tree is
  ...
  /tmp/asoft_ai_services_src``. This is a transitional path used until
  ASOFT is published; it is marked with TODOs in the templates so
...
# directly rather than via ``python -m Capabilities.workspace._mcp_gateway``
# so Python does not auto-import ``Capabilities.workspace.__init__`` (which
...
IMAGE_REPO = "ASOFT-workspace"

# ── template loading ───────────────────────────────────────────────

_TEMPLATE_PKG = "Capabilities.workspace._docker"
...
def render_dockerfile(
    *,
    ...
    Args:
        ...
        install_asoft_block: Pre-rendered block (no surrounding
            blank lines) that installs ``ASOFT`` into the gateway
    ...
    return _read_template(_DOCKERFILE_TEMPLATE).format(
        ...
        install_asoft_block=install_asoft_block.rstrip() + "\n",
    )
...
def compute_image_tag(
    ...
    Args:
        ...
        copy_files: Mapping of context-relative filename → bytes for
            ...
            payloads (e.g. the ASOFT source tree in dev mode) are
            represented as a single synthetic key whose value is the
            tree's content hash.

    Returns:
        Tag of the form ``ASOFT-workspace:<12 hex chars>``.
    """
    h = hashlib.sha256()
...
def prepare_build_context(
    *,
    ...
    Writes Dockerfile, ``requirements.txt`` and the ASOFT payload
    ...
    if released:
        version = _asoft_version()
        install_block = _read_template(
            _DOCKERFILE_INSTALL_PYPI_TEMPLATE,
        ).format(asoft_ai_services_version=version)
        source_root: Path | None = None
    else:
        # TODO(release): drop this branch once ASOFT is on PyPI; the
        ...
        source_root = _asoft_source_root()
    ...
    dockerfile_text = render_dockerfile(
        ...
        container_workdir=container_workdir,
        node_version=node_version,
        install_asoft_block=install_block,
    )
    ...
    if source_root is not None:
        ...
        copy_files["asoft_ai_services_src/"] = _hash_directory(source_root)
