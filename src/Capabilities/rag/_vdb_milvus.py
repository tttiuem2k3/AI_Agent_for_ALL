...
class MilvusLiteStore(VectorStoreBase):
    ...
    :class:`~ASOFT.rag.Chunk`, and the chunk metadata in a JSON
    field used for flat equality filtering.

    .. note:: Install optional dependencies with
        ``pip install asoft-ai-services[vdb-milvus]``.
    ...
    def __init__(
        ...
        Args:
            uri (`str`, defaults to ``"./ASOFT_milvus_lite.db"``):
    ...
    def get_client(self) -> "MilvusClient":
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "MilvusLiteStore requires pymilvus. Install with "
                    "asoft-ai-services[vdb-milvus].",
                ) from exc
