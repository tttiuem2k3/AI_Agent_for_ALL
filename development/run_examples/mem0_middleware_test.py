...
The mem0 client itself is mocked — we only exercise the ASOFT AI Services hook
...
ASOFT AI Services and mem0. The OSS ``Memory`` and Platform ``MemoryClient``
...
from Runtime.message import HintBlock, Msg, TextBlock, UserMsg
from Runtime.middleware import Mem0Middleware
from Runtime.middleware._longterm_memory._mem0._utils import (
    _extract_memory_texts,
    _extract_query_text,
)
from Providers.modelLLM.model import ChatResponse
from Capabilities.tool import Toolkit
...
class TestExtractQueryText(IsolatedAsyncioTestCase):
    ...
    def test_resumption_events_return_none(self) -> None:
        """HITL resumption events should not trigger middleware IO."""
        self.assertIsNone(
            _extract_query_text(
                UserConfirmResultEvent(
                    reply_id="reply",
...
class TestExtractMemoryTexts(IsolatedAsyncioTestCase):
    ...
    def test_dict_with_results(self) -> None:
        ...
        raw = {"results": [{"middleware": "a"}, {"middleware": "b"}]}
    ...
    def test_plain_list_of_dicts(self) -> None:
        ...
        self.assertEqual(
            _extract_memory_texts([{"middleware": "only"}]),
            ["only"],
        )
...
class TestConstructorValidation(IsolatedAsyncioTestCase):
    ...
    def test_client_wins_over_other_backend_kwargs_with_warning(
        ...
        # The fake client wired through unchanged — no ASOFT AI Services-
...
class TestStaticControlMode(IsolatedAsyncioTestCase):
    ...
    async def test_retrieve_inject_write(self) -> None:
        ...
        fake = _FakeAsyncMem0Client(
            search_return={"results": [{"middleware": "alice loves coffee"}]},
        )
        mw = Mem0Middleware(
            client=fake,
            user_id="alice",
        ...
        # 4. middleware appended to the agent's persistent context as an
        #    assistant-role hint note named "middleware". Formatters convert
        #    HintBlock content into a user message before provider calls.
        memory_msgs = []
        for msg in agent.state.context:
            if getattr(msg, "name", None) == "middleware":
        ...
        delivered = [
            ...
            if getattr(m, "name", None) == "middleware"
        ]
    ...
    async def test_memory_message_lands_after_user_message(self) -> None:
        """Following the ASOFT AI Services long-term-memory contract: the middleware
        note is inserted right AFTER the new user input lands in the
        agent context, not before."""
        fake = _FakeAsyncMem0Client(
            search_return={"results": [{"middleware": "saved fact"}]},
        )
        mw = Mem0Middleware(
            client=fake,
            user_id="alice",
        ...
        user_idx = next(
            ...
            if r == "user" and n != "middleware"
        )
        memory_idx = next(
            i for i, (_, n) in enumerate(roles_and_names) if n == "middleware"
        )
        self.assertGreater(
            memory_idx,
            user_idx,
            f"middleware note at {memory_idx} should come after user msg "
            f"at {user_idx}: {roles_and_names}",
        )

    async def test_no_memories_no_injection(self) -> None:
        """Empty search results should not add a middleware hint message."""
        ...
        # System prompt unchanged, no synthetic middleware msg present.
        msgs = self.model.last_call_messages
        self.assertEqual(msgs[0].get_text_content(), "base system prompt")
        for m in msgs:
            self.assertNotEqual(getattr(m, "name", None), "middleware")
        self.assertEqual(len(fake.search_calls), 1)
...
class TestAgentControlMode(IsolatedAsyncioTestCase):
    """Tools are listed, but no automatic middleware hook behavior."""
    ...
    async def test_tools_listed_and_hint_in_prompt(self) -> None:
        ...
        fake = _FakeAsyncMem0Client(
            search_return={"results": [{"middleware": "found"}]},
        )
        mw = Mem0Middleware(
            client=fake,
            user_id="alice",
        ...
        self.assertIn("Long-term middleware", prompt_text)
        self.assertIn("search_memory", prompt_text)
        self.assertIn("add_memory", prompt_text)
        # No synthetic middleware message was injected in agent_control mode.
        for m in msgs:
            self.assertNotEqual(getattr(m, "name", None), "middleware")
    ...
    async def test_search_memory_tool_invokes_mem0(self) -> None:
        ...
        fake = _FakeAsyncMem0Client(
            search_return={
                "results": [
                    {"middleware": "first fact"},
                    {"middleware": "second fact"},
                ],
            },
        )
        mw = Mem0Middleware(
        ...
        # ASOFT AI Services 1.x: a LIST of keywords, each issued as an
    ...
    async def test_tools_auto_allow_permission(self) -> None:
        ...
        from Capabilities.permission import PermissionBehavior
    ...
    async def test_search_memory_dedupes_across_keywords(self) -> None:
        ...
        fake = _FakeAsyncMem0Client(
            search_return={
                "results": [{"middleware": "shared"}, {"middleware": "unique"}],
            },
        )
    ...
    async def test_search_memory_failure_returns_error_chunk(self) -> None:
        ...
        from Runtime.message import ToolResultState
        from Capabilities.tool import ToolChunk
    ...
    async def test_add_memory_does_not_persist_thinking(self) -> None:
        ...
        sent to mem0, so the stored middleware stays clean of agent
        self-narration."""
        fake = _FakeAsyncMem0Client()
        # First attempt extracts something → no fallback.
        fake.add = AsyncMock(
            return_value={"results": [{"id": "m1", "middleware": "saved"}]},
        )
        mw = Mem0Middleware(client=fake, user_id="alice", mode="agent_control")
        agent = await self._agent(mw)
        await agent.reply(UserMsg("user", "hi"))
...
class TestBothMode(IsolatedAsyncioTestCase):
    ...
    async def test_memory_msg_and_tool_hint_both_present(self) -> None:
        """Both mode should inject middleware and expose middleware tools."""
        model = RecordingMockModel(context_size=100_000)
        fake = _FakeAsyncMem0Client(
            search_return={"results": [{"middleware": "auto-injected"}]},
        )
        ...
        memory_msgs = [m for m in msgs if getattr(m, "name", None) == "middleware"]
        self.assertEqual(len(memory_msgs), 1)
        memory_hints = memory_msgs[0].get_content_blocks("hint")
        self.assertEqual(len(memory_hints), 1)
