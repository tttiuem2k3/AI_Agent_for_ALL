...
from Service.app.middleware import AGUIProtocolMiddleware
...
from Runtime.message import ToolCallBlock, ToolResultBlock, ToolResultState
...
class AGUIProtocolLifecycleTest(IsolatedAsyncioTestCase):
    ...
    async def test_terminal_run_error_is_preserved(self) -> None:
        """Test sanitized service failure -> AGUI RUN_ERROR."""
        event = RunErrorEvent(
            session_id="sess_1",
            code="model_execution_failed",
            message="The AI model could not complete this run.",
            retryable=True,
        )
        result = self.mw._convert_to_protocol(event)

        self.assertEqual(result["type"], "RUN_ERROR")
        self.assertEqual(result["code"], "model_execution_failed")
