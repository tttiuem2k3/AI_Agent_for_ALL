...
class TestDashScopeFormatter(IsolatedAsyncioTestCase):
    ...
    @patch(
        "Providers.modelLLM.formatter._formatter_base.shortuuid.uuid",
    ...
    async def test_chat_formatter_thinking_dropped_without_flag(self) -> None:
        """ThinkingBlock is silently dropped when app/x-thinking is
    ...
    async def test_chat_formatter_thinking_becomes_reasoning_content(
        ...
        """ThinkingBlock becomes reasoning_content when app/x-thinking
        is in input_types."""
        fmt = DashScopeChatFormatter(
            input_types=["text/plain", "app/x-thinking"],
        )
        msgs = [
            AssistantMsg(
                name="assistant",
    ...
    async def test_multiagent_formatter_thinking_in_tool_sequence(
        ...
        reasoning_content when app/x-thinking is in input_types."""
        fmt = DashScopeMultiAgentFormatter(
            input_types=["text/plain", "app/x-thinking"],
        )
        tc = ToolCallBlock(
