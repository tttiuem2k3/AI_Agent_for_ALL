# -*- coding: utf-8 -*-
"""Unit tests for the tracing module using an in-middleware OTel exporter."""
import json
...
from Providers.modelLLM.model import ChatResponse, ChatUsage
from Capabilities.permission import (
    PermissionContext,
    PermissionDecision,
    PermissionBehavior,
)
from Capabilities.tool import Toolkit, ToolBase
from Runtime.middleware import TracingMiddleware
...
class TracingTest(IsolatedAsyncioTestCase):
    ...
    The in-middleware exporter is set up once per class (setUpClass) because
    ...
    def setUpClass(cls) -> None:
        """Configure an in-middleware OTel provider once for the whole class."""
    ...
    async def test_invoke_agent_span_has_reply_id(self) -> None:
        ...
        span_attrs = dict(agent_spans[0].attributes or {})
        self.assertIn(
            "ASOFT.agent.reply_id",
            span_attrs,
            "invoke_agent span should have ASOFT.agent.reply_id",
        )
    ...
    async def test_hitl_first_call_has_hitl_pending_attribute(self) -> None:
        ...
        ASOFT.agent.hitl_pending_tools."""
        hitl_agent = Agent(
            name="hitl-agent",
            system_prompt="You are a test assistant.",
            model=self.model,
        ...
        self.assertIn(
            "ASOFT.agent.hitl_pending_tools",
            span_attrs,
            "First HITL span should carry ASOFT.agent.hitl_pending_tools",
        )
        pending = json.loads(span_attrs["ASOFT.agent.hitl_pending_tools"])
        self.assertEqual(pending, ["get_weather"])

    async def test_hitl_spans_share_reply_id(self) -> None:
        """Both HITL calls must share the same ASOFT.agent.reply_id."""
        hitl_agent = Agent(
            name="hitl-agent2",
            system_prompt="You are a test assistant.",
            model=self.model,
        ...
        first_spans = self._spans_by_name("invoke_agent")
        first_reply_ids = {
            dict(s.attributes or {}).get("ASOFT.agent.reply_id")
            for s in first_spans
            if "ASOFT.agent.reply_id" in (s.attributes or {})
        }
    ...
    async def test_hitl_second_call_has_incoming_event_type(self) -> None:
        ...
        span_attrs = dict(agent_spans[0].attributes or {})
        self.assertEqual(
            span_attrs.get("ASOFT.agent.incoming_event_type"),
            "user_confirm_result",
    ...
    async def test_external_execution_first_call_has_pending_attribute(
        ...
        ASOFT.agent.external_execution_pending_tools."""
        ext_agent = Agent(
            name="ext-agent",
            system_prompt="You are a test assistant.",
            model=self.model,
        ...
        pending = json.loads(
            span_attrs["ASOFT.agent.external_execution_pending_tools"],
        )
        self.assertEqual(pending, ["get_weather"])
    ...
    async def test_external_execution_second_call_has_synthetic_tool_span(
        ...
        span_attrs = dict(tool_spans[0].attributes or {})
        self.assertEqual(
            span_attrs.get("ASOFT.agent.is_external_execution"),
            True,
