...
from Runtime.message import TextBlock, ToolCallBlock, ThinkingBlock
from Providers.modelLLM.model import OpenAIResponseModel
from Providers.credential import OpenAICredential
from Capabilities.tool import ToolChoice
...
class TestOpenAIResponseStream(IsolatedAsyncioTestCase):
    ...
    @patch("openai.AsyncClient")
    async def test_stream_incomplete_max_output_tokens_returns_partial(
        self,
        mock_client_cls: MagicMock,
    ) -> None:
        """A token-limited stream closes with its accumulated partial text."""
        incomplete_resp = MagicMock()
        incomplete_resp.id = "resp-incomplete"
        incomplete_resp.output = None
        incomplete_resp.usage = MagicMock()
        incomplete_resp.usage.input_tokens = 12
        incomplete_resp.usage.output_tokens = 8
        incomplete_resp.usage.input_tokens_details = None
        incomplete_resp.incomplete_details = MagicMock(
            reason="max_output_tokens",
        )

        events = [
            _make_event("response.output_text.delta", delta="Partial answer"),
            _make_event("response.incomplete", response=incomplete_resp),
        ]
        ...
        self.assertEqual(responses[-1].content[0].text, "Partial answer")
        self.assertEqual(responses[-1].usage.input_tokens, 12)
    ...
    @patch("openai.AsyncClient")
    async def test_stream_incomplete_content_filter_raises(
        self,
        mock_client_cls: MagicMock,
    ) -> None:
        """Safety-filtered partial output is not treated as successful."""
        incomplete_resp = MagicMock()
        incomplete_resp.id = "resp-filtered"
        incomplete_resp.incomplete_details = MagicMock(
        ...
        gen = await self.model([])
        with self.assertRaisesRegex(RuntimeError, "ended incomplete"):
            _ = [response async for response in gen]
    ...
    async def test_stream_failed_raises(
        ...
        """Provider failure is surfaced without leaking provider details."""
        mock_client_cls.return_value.responses.create = AsyncMock(
            return_value=_MockAsyncEventStream(
                [_make_event("response.failed")],
            ),
        )
    ...
    async def test_stream_without_terminal_event_raises(
        ...
        """A truncated stream cannot silently violate ChatResponse contract."""
        mock_client_cls.return_value.responses.create = AsyncMock(
            return_value=_MockAsyncEventStream(
