...
from Service.app._service import SubagentHitlProjector
from Service.app.storage import AgentRecord, AgentData
from Service.app.storage._model._session import SessionRecord
from Service.app.storage._model._team import TeamRecord
...
class _FakeProjection:
    ...
    / ``publish`` / ``list``) over an in-middleware ``{(sid, kind): {eid:
    ...
    async def upsert(
        ...
        """Record an upsert in the in-middleware store."""
    ...
    async def delete(
        ...
        """Remove an entry from the in-middleware store."""
