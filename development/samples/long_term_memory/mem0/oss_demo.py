# -*- coding: utf-8 -*-
"""Mem0 middleware demo (open-source mem0, ASOFT AI Services-driven).

Drives two independent agent sessions for the same ``user_id`` so
mem0's cross-session middleware effect is visible. Each turn streams
events from ``agent.reply_stream`` and prints the ones that matter:

- ``[mem0 → context (static)]`` — the middleware note the middleware
...
from Providers.modelBSN.embedding import DashScopeEmbeddingModel
...
def _injected_memory_bullets(agent: Agent) -> list[str]:
    """Extract the bullet lines from the middleware note the middleware
...
async def _run_turn(agent: Agent, user_msg: UserMsg) -> str:
    ...
    1. ``ReplyStartEvent`` fires right after the agent ingests the new
       user input AND the middleware has appended any retrieved middleware
    ...
    async for ev in agent.reply_stream(inputs=user_msg):
        if isinstance(ev, ReplyStartEvent) and not memory_announced:
            ...
            print(
                ...
                f"{len(injected)} middleware note(s):",
            )
            for b in injected:
                print(f"  ← {b}")
...
async def main() -> None:
    ...
    # `stream` can be True or False — the ASOFT AI Services→mem0 adapter
    # drains an async generator and uses the last chunk (which carries
    # the full accumulated content per ASOFT AI Services's streaming contract).
    ...
    # ``.embedder`` to route through your ASOFT AI Services models.
    #
