...
`ASOFT AI Services.agent.Agent`. Drives two consecutive agent sessions for
...
self-hosted via local Qdrant) with mem0 driven by ASOFT AI Services's own
...
```bash
# mem0 is an optional ASOFT AI Services dependency — pull it via the extra:
pip install "asoft-ai-services[mem0]"
...
```python
# 1. Models — build a local OSS AsyncMemory wired to your ASOFT AI Services
...
#    OVERWRITTEN by the ASOFT AI Services adapter built from your model.
...
| — | ✓ | ✓ | — | Wrap + override `.llm` with the ASOFT AI Services adapter; keep `.embedder` from `mem0_config`. |
...
| — | — | ✓ | ✓ | Build a fresh `MemoryConfig` (mem0 defaults for vector store / history DB) with the ASOFT AI Services adapters wired in. |
...
- **`client` wins** lets one `Mem0Middleware(...)` call
  shape work for both library callers (who pass ASOFT AI Services models)
...
the legacy long-term-memory behavior:
...
on-demand search / save. This is the default ASOFT AI Services behavior.
...
chat_model = ...  # shared ASOFT AI Services ChatModelBase
emb_model = ...  # shared ASOFT AI Services EmbeddingModelBase
...
- `user_id` is the authenticated caller, injected by `ASOFT AI Services.app`
  via `get_current_user_id` (currently from `X-User-ID` header; will
  can become JWT-based when ERPX authentication is integrated). You forward it
  straight
...
## Notes on the ASOFT AI Services-as-mem0-backend path

When you pass `chat_model` + `embedding_model`, the middleware
internally:

1. Registers `ASOFTLLM` / `ASOFTEmbedding` in mem0's factory
   dicts under provider name `"ASOFT AI Services"`.
2. Substitutes `LlmConfig` / `EmbedderConfig` with subclasses whose
   validator allows `"ASOFT AI Services"` (mem0's stock validator hardcodes a
...
3. Builds an `AsyncMemory` whose `.llm` and `.embedding_model` route
   through the ASOFT AI Services adapters.
4. Bridges mem0's sync API onto ASOFT AI Services's async models via a
